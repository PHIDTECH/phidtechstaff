...
import { Film, Plus, Search, Upload, Trash2, Edit, AlertCircle, CheckCircle, Download } from "lucide-react";
...
interface MoviesCustomer {
  ...
  email?: string; address?: string; subscriptionType?: string;
  preferredGenre?: string; contactPerson?: string; notes?: string; createdAt: string;
}

const SUBSCRIPTION_TYPES = ["Monthly", "Quarterly", "Annual", "Pay-Per-View", "Free Trial", "Corporate", "Other"];
const GENRES = ["Action", "Comedy", "Drama", "Documentary", "Animation", "Horror", "Romance", "Sci-Fi", "Thriller", "Swahili Films", "Local Content", "Other"];

const emptyForm = (): Omit<MoviesCustomer, "id" | "createdAt"> => ({
  companyId: "", name: "", phone: "", email: "", address: "",
  subscriptionType: "", preferredGenre: "", contactPerson: "", notes: "",
});
...
export default function MoviesCustomersPage() {
  const [customers, setCustomers] = useState<MoviesCustomer[]>([]);
  const [companies, setCompanies] = useState<{id:string;name:string}[]>([]);
  const [cid, setCid] = useState(""); const [search, setSearch] = useState(""); const [loading, setLoading] = useState(true);
  const [showDialog, setShowDialog] = useState(false); const [editItem, setEditItem] = useState<MoviesCustomer | null>(null);
  ...
  const [bulkPreview, setBulkPreview] = useState<Omit<MoviesCustomer,"id"|"createdAt">[]>([]);
  ...
  const load = async () => {
    ...
    try { const r = await fetch("/api/movies-customers", { cache: "no-store" }); if (r.ok) setCustomers(await r.json()); } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const visible = customers.filter(c => {
    ...
    return ok && (!q || c.name.toLowerCase().includes(q) || c.phone.includes(q) ||
      (c.subscriptionType ?? "").toLowerCase().includes(q) || (c.preferredGenre ?? "").toLowerCase().includes(q));
  });

  const openAdd = () => { setEditItem(null); setForm({ ...emptyForm(), companyId: cid }); setFormError(""); setShowDialog(true); };
  const openEdit = (c: MoviesCustomer) => {
    setEditItem(c);
    setForm({ companyId: c.companyId, name: c.name, phone: c.phone, email: c.email ?? "", address: c.address ?? "",
      subscriptionType: c.subscriptionType ?? "", preferredGenre: c.preferredGenre ?? "", contactPerson: c.contactPerson ?? "", notes: c.notes ?? "" });
  ...
  const save = async () => {
    ...
    try {
      const r = await fetch("/api/movies-customers", { method: editItem ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editItem ? { ...form, id: editItem.id } : form) });
  ...
  const remove = async (id: string) => { await fetch(`/api/movies-customers?id=${id}`, { method: "DELETE" }); setDeleteId(null); await load(); };
  ...
  const parseBulk = (raw: string) => {
    ...
    const parsed: Omit<MoviesCustomer,"id"|"createdAt">[] = []; const errs: string[] = [];
    lines.slice(1).forEach((l, i) => {
      ...
      parsed.push({ companyId: bulkCompany, name, phone, email: cols[idx("email")]?.replace(/^"|"$/g,"").trim()||"",
        subscriptionType: cols[idx("subscription type")]?.replace(/^"|"$/g,"").trim()||"",
        preferredGenre: cols[idx("preferred genre")]?.replace(/^"|"$/g,"").trim()||"",
  ...
  const submitBulk = async () => {
    ...
    try {
      const r = await fetch("/api/movies-customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bulkPreview.map(p => ({ ...p, companyId: bulkCompany }))) });
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-600 flex items-center justify-center"><Film className="w-5 h-5 text-white" /></div>
            <div><h1 className="text-xl font-bold text-gray-900">Movies Customers</h1><p className="text-sm text-gray-500">Manage movie subscription and cinema clients</p></div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <ImportExport
              label="Movies Customers"
              rows={customers as unknown as Record<string, unknown>[]}
              onImport={async (rows) => {
                const res = await fetch("/api/bulk-import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dbKey: "movies_customers", records: rows }) });
                const data = await res.json();
                await load();
                return { imported: data.imported ?? 0, errors: data.errors ?? [] };
              }}
            />
            <Button size="sm" className="bg-amber-600 hover:bg-amber-700 text-white" onClick={openAdd}><Plus className="w-4 h-4 mr-1.5" /> Add Customer</Button>
          </div>
        </div>

        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-56"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" /><Input placeholder="Search by name, phone, or subscription…" value={search} onChange={e => setSearch(e.target.value)} className="pl-9" /></div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          : visible.length === 0 ? (
            <div className="flex flex-col items-center py-16 gap-3"><Film className="w-10 h-10 text-gray-200" /><p className="text-sm text-gray-500">No movies customers yet</p><Button size="sm" variant="outline" onClick={openAdd}><Plus className="w-4 h-4 mr-1" /> Add First</Button></div>
          ) : (
            <Table>
              <TableHeader><TableRow>
                <TableHead>#</TableHead><TableHead>Name</TableHead><TableHead>Phone</TableHead>
                <TableHead>Subscription</TableHead><TableHead>Genre</TableHead>
              ...
              <TableBody>
                {visible.map((c, i) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>{c.subscriptionType ? <span className="px-2 py-0.5 text-xs rounded-full bg-amber-50 text-amber-700">{c.subscriptionType}</span> : "—"}</TableCell>
                    <TableCell className="text-sm text-gray-600">{c.preferredGenre || "—"}</TableCell>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>{editItem ? "Edit Customer" : "Add Movies Customer"}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Full Name *</label><Input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="e.g. Jane Subscriber" /></div>
                ...
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Subscription Type</label>
                  <Select value={form.subscriptionType || "__none__"} onValueChange={v => setForm(f => ({...f, subscriptionType: v === "__none__" ? "" : v}))}>
                    <SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger>
                    <SelectContent><SelectItem value="__none__">— None —</SelectItem>{SUBSCRIPTION_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Preferred Genre</label>
                  <Select value={form.preferredGenre || "__none__"} onValueChange={v => setForm(f => ({...f, preferredGenre: v === "__none__" ? "" : v}))}>
                    <SelectTrigger><SelectValue placeholder="Select genre" /></SelectTrigger>
                    <SelectContent><SelectItem value="__none__">— None —</SelectItem>{GENRES.map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Contact Person</label><Input value={form.contactPerson} onChange={e => setForm(f => ({...f, contactPerson: e.target.value}))} placeholder="e.g. John Rep" /></div>
            ...
            <DialogFooter>
              ...
              <Button onClick={save} disabled={saving} className="bg-amber-600 hover:bg-amber-700 text-white">{saving ? "Saving…" : editItem ? "Save Changes" : "Add Customer"}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showBulk} onOpenChange={setShowBulk}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="flex items-center gap-2"><Upload className="w-5 h-5 text-amber-600" /> Bulk Import Movies Customers</DialogTitle></DialogHeader>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-lg">
                ...
                <div className="flex-1"><p className="text-sm font-medium text-amber-800">CSV Template</p><p className="text-xs text-amber-600">Required: <strong>Name, Phone</strong>. Optional: Subscription Type, Preferred Genre, Contact Person, Email, Address, Notes</p></div>
                <Button size="sm" variant="outline" onClick={() => { const csv = `Name,Phone,Subscription Type,Preferred Genre,Contact Person,Email,Address,Notes\nJane Sub,255712345678,Monthly,Drama,John Rep,jane@example.com,Dar es Salaam,`; const a = document.createElement("a"); a.href = URL.createObjectURL(new Blob([csv],{type:"text/csv"})); a.download="movies_customers_template.csv"; a.click(); }}>Download</Button>
              </div>
              ...
              <div><label className="text-xs font-medium text-gray-600 mb-1 block">Or paste CSV data</label><textarea rows={4} value={bulkText} onChange={e => { setBulkText(e.target.value); parseBulk(e.target.value); }} className="w-full border border-gray-200 rounded-lg p-3 text-sm font-mono resize-y focus:outline-none focus:ring-1 focus:ring-amber-400" /></div>
              ...
              {bulkPreview.length > 0 && <div><p className="text-xs font-medium text-gray-600 mb-1.5">{bulkPreview.length} customers ready:</p><div className="border border-gray-200 rounded-lg max-h-40 overflow-y-auto divide-y divide-gray-50">{bulkPreview.map((p,i)=><div key={i} className="px-3 py-2"><p className="text-sm font-medium">{p.name}</p><p className="text-xs text-gray-500">{p.phone}{p.subscriptionType ? ` · ${p.subscriptionType}` : ""}{p.preferredGenre ? ` · ${p.preferredGenre}` : ""}</p></div>)}</div></div>}
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setShowBulk(false)}>Close</Button>
              <Button onClick={submitBulk} disabled={bulkSaving || !bulkPreview.length || !bulkCompany} className="bg-amber-600 hover:bg-amber-700 text-white">{bulkSaving ? "Importing…" : `Import ${bulkPreview.length || ""} Customers`}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
