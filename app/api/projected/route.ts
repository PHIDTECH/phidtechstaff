...
export async function GET(req: NextRequest) {
  ...
  let list = readDb<Projection[]>("projections", []);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const list = readDb<Projection[]>("projections", []);
    const item: Projection = { ...body, id: body.id ?? `proj-${Date.now()}`, createdAt: body.createdAt ?? new Date().toISOString() };
    list.push(item);
    writeDb("projections", list);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    writeDb("projections", readDb<Projection[]>("projections", []).filter(p => p.id !== id));
