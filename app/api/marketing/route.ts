...
export async function GET() {
  return NextResponse.json(readDb<Campaign[]>("marketing", []));
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const list = readDb<Campaign[]>("marketing", []);
    const item: Campaign = { ...body, id: body.id ?? `camp-${Date.now()}` };
    list.push(item);
    writeDb("marketing", list);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    writeDb("marketing", readDb<Campaign[]>("marketing", []).filter(x => x.id !== id));
