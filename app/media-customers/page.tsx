...
import { Radio, Plus, Search, Upload, Trash2, Edit, AlertCircle, CheckCircle, Download } from "lucide-react";
...
const MEDIA_TYPES = ["TV", "Radio", "Print", "Online / Digital", "Podcast", "Other"];

const emptyForm = (): Omit<MediaCustomer, "id" | "createdAt"> => ({
  companyId: "", name: "", phone: "",
  email: "", address: "", mediaOutlet: "",
  mediaType: "", contactPerson: "", notes: "",
});
...
export default function MediaCustomersPage() {
  const [customers, setCustomers] = useState<MediaCustomer[]>([]);
  const [companies, setCompanies] = useState<{id:string;name:string}[]>([]);
  const [cid,       setCid]       = useState("");
  const [search,    setSearch]    = useState("");
  const [loading,   setLoading]   = useState(true);
  const [showDialog, setShowDialog] = useState(false);
  const [editItem,   setEditItem]   = useState<MediaCustomer | null>(null);
  ...
  const [bulkPreview,setBulkPreview]= useState<Omit<MediaCustomer,"id"|"createdAt">[]>([]);
  ...
  const load = async () => {
    ...
    try { const r = await fetch("/api/media-customers", { cache: "no-store" }); if (r.ok) setCustomers(await r.json()); } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/media-customers", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const remove = async (id: string) => {
    await fetch(`/api/media-customers?id=${id}`, { method: "DELETE" });
  ...
  const parseBulk = (raw: string) => {
    ...
    const nameIdx = idx("name"); const phoneIdx = idx("phone");
    if (nameIdx === -1 || phoneIdx === -1) { setBulkError(`Missing required columns. Header must include: Name, Phone. Found: ${header.join(", ")}`); return; }
    const parsed: Omit<MediaCustomer,"id"|"createdAt">[] = [];
    const errors: string[] = [];
    lines.slice(1).forEach((line, i) => {
      ...
      parsed.push({ companyId: bulkCompany, name, phone,
        email: cols[idx("email")]?.replace(/^"|"$/g,"").trim() || "",
        mediaOutlet: cols[idx("media outlet")]?.replace(/^"|"$/g,"").trim() || "",
        mediaType: cols[idx("media type")]?.replace(/^"|"$/g,"").trim() || "",
  ...
  const submitBulk = async () => {
    ...
    try {
      const r = await fetch("/api/media-customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bulkPreview.map(p => ({ ...p, companyId: bulkCompany }))) });
  ...
  const downloadTemplate = () => {
    const csv = `Name,Phone,Media Outlet,Media Type,Contact Person,Email,Address,Notes\nJohn Doe,255712345678,TBC TV,TV,Jane Editor,john@tbc.co.tz,Dar es Salaam,Prime time slot`;
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = "media_customers_template.csv"; a.click();
  };
      ...
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Media Customers</h1>
              <p className="text-sm text-gray-500">Register and manage media house customers</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <ImportExport
              ...
              onImport={async (rows) => {
                const res = await fetch("/api/bulk-import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dbKey: "media_customers", records: rows }) });
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-56">
            ...
            <Input placeholder="Search by name, phone, or media outlet…" value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-16 text-center gap-3">
              ...
              <p className="text-sm text-gray-500">No media customers yet</p>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Media Outlet</TableHead><TableHead>Media Type</TableHead>
                  <TableHead>Contact Person</TableHead><TableHead>Company</TableHead>
              ...
              <TableBody>
                {visible.map((c, i) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-sm text-gray-600">{c.mediaOutlet || "—"}</TableCell>
                    <TableCell>
                      {c.mediaType ? <span className="px-2 py-0.5 text-xs rounded-full bg-sky-50 text-sky-700">{c.mediaType}</span> : "—"}
                    </TableCell>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>{editItem ? "Edit Customer" : "Add Media Customer"}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Full Name *</label><Input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="e.g. John Doe" /></div>
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Phone *</label><Input value={form.phone} onChange={e => setForm(f => ({...f, phone: e.target.value}))} placeholder="e.g. 255712345678" /></div>
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Media Outlet</label><Input value={form.mediaOutlet} onChange={e => setForm(f => ({...f, mediaOutlet: e.target.value}))} placeholder="e.g. TBC TV" /></div>
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Media Type</label>
                  <Select value={form.mediaType || "__none__"} onValueChange={v => setForm(f => ({...f, mediaType: v === "__none__" ? "" : v}))}>
                    <SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger>
                    <SelectContent><SelectItem value="__none__">— None —</SelectItem>{MEDIA_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Contact Person</label><Input value={form.contactPerson} onChange={e => setForm(f => ({...f, contactPerson: e.target.value}))} placeholder="e.g. Jane Editor" /></div>
            ...
            <DialogFooter>
              ...
              <Button onClick={save} disabled={saving} className="bg-sky-600 hover:bg-sky-700 text-white">{saving ? "Saving…" : editItem ? "Save Changes" : "Add Customer"}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showBulk} onOpenChange={setShowBulk}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="flex items-center gap-2"><Upload className="w-5 h-5 text-sky-600" /> Bulk Import Media Customers</DialogTitle></DialogHeader>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-sky-50 border border-sky-100 rounded-lg">
                <Download className="w-4 h-4 text-sky-600 shrink-0" />
                <div className="flex-1">
                  <p className="text-sm font-medium text-sky-800">CSV Template</p>
                  <p className="text-xs text-sky-600">Required: <strong>Name, Phone</strong>. Optional: Media Outlet, Media Type, Contact Person, Email, Address, Notes</p>
                </div>
                <Button size="sm" variant="outline" onClick={downloadTemplate} className="shrink-0">Download</Button>
              </div>
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Upload CSV</label>
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Or paste CSV data</label>
                <textarea rows={5} value={bulkText} onChange={e => { setBulkText(e.target.value); parseBulk(e.target.value); }}
                  placeholder={"Name,Phone,Media Outlet,...\nJohn Doe,255712345678,TBC TV,..."}
                  className="w-full border border-gray-200 rounded-lg p-3 text-sm font-mono resize-y focus:outline-none focus:ring-1 focus:ring-sky-400" />
              </div>
              {bulkError && <div className="flex items-start gap-2 bg-red-50 border border-red-200 rounded-lg px-3 py-2.5"><AlertCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" /><pre className="text-xs text-red-600 whitespace-pre-wrap">{bulkError}</pre></div>}
              {bulkSuccess && <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-3 py-2.5"><CheckCircle className="w-4 h-4 text-green-500 shrink-0" /><p className="text-sm text-green-700 font-medium">{bulkSuccess}</p></div>}
                  ...
                  <div className="border border-gray-200 rounded-lg max-h-48 overflow-y-auto divide-y divide-gray-50">
                    {bulkPreview.map((p, i) => (
                      <div key={i} className="px-3 py-2">
                        ...
                        <p className="text-xs text-gray-500">{p.phone}{p.mediaOutlet ? ` · ${p.mediaOutlet}` : ""}{p.mediaType ? ` · ${p.mediaType}` : ""}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={submitBulk} disabled={bulkSaving || bulkPreview.length === 0 || !bulkCompany} className="bg-sky-600 hover:bg-sky-700 text-white">
