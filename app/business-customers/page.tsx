...
import { Building2, Plus, Search, Upload, Trash2, Edit, AlertCircle, CheckCircle, Download } from "lucide-react";
...
const BUSINESS_TYPES = ["Sole Proprietorship", "Partnership", "Limited Company", "NGO / NPO", "Government", "Cooperative", "Other"];

const emptyForm = (): Omit<BusinessCustomer, "id" | "createdAt"> => ({
  companyId: "", name: "", phone: "",
  email: "", address: "", businessName: "",
  businessType: "", registrationNo: "", contactPerson: "", notes: "",
});
...
export default function BusinessCustomersPage() {
  ...
  const [editItem,   setEditItem]   = useState<BusinessCustomer | null>(null);
  ...
  const [bulkPreview,setBulkPreview]= useState<Omit<BusinessCustomer,"id"|"createdAt">[]>([]);
  ...
  const load = async () => {
    ...
    try { const r = await fetch("/api/business-customers", { cache: "no-store" }); if (r.ok) setCustomers(await r.json()); } catch {}
    setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  const visible = customers.filter(c => {
    ...
    const matchQ = !q || c.name.toLowerCase().includes(q) || c.phone.includes(q) ||
      (c.businessName ?? "").toLowerCase().includes(q) || (c.businessType ?? "").toLowerCase().includes(q) ||
  ...
  const openEdit = (c: BusinessCustomer) => {
    setEditItem(c);
    setForm({ companyId: c.companyId, name: c.name, phone: c.phone, email: c.email ?? "", address: c.address ?? "",
      businessName: c.businessName ?? "", businessType: c.businessType ?? "",
      registrationNo: c.registrationNo ?? "", contactPerson: c.contactPerson ?? "", notes: c.notes ?? "" });
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch("/api/business-customers", { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const remove = async (id: string) => {
    await fetch(`/api/business-customers?id=${id}`, { method: "DELETE" });
  ...
  const parseBulk = (raw: string) => {
    ...
    if (nameIdx === -1 || phoneIdx === -1) { setBulkError(`Missing required columns: Name, Phone. Found: ${header.join(", ")}`); return; }
    const parsed: Omit<BusinessCustomer,"id"|"createdAt">[] = [];
    const errors: string[] = [];
    lines.slice(1).forEach((line, i) => {
      ...
      parsed.push({ companyId: bulkCompany, name, phone,
        ...
        businessType: cols[idx("business type")]?.replace(/^"|"$/g,"").trim() || "",
        registrationNo: cols[idx("registration no")]?.replace(/^"|"$/g,"").trim() || "",
  ...
  const submitBulk = async () => {
    ...
    try {
      const r = await fetch("/api/business-customers", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bulkPreview.map(p => ({ ...p, companyId: bulkCompany }))) });
  ...
  const downloadTemplate = () => {
    const csv = `Name,Phone,Business Name,Business Type,Registration No,Contact Person,Email,Address,Notes\nAcme Ltd,255712345678,Acme Enterprises,Limited Company,REG-001,John CEO,acme@example.com,Dar es Salaam,Key client`;
    const blob = new Blob([csv], { type: "text/csv" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = "business_customers_template.csv"; a.click();
  };
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Company &amp; Business Customers</h1>
              <p className="text-sm text-gray-500">Manage corporate and business clients</p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <ImportExport
              ...
              onImport={async (rows) => {
                const res = await fetch("/api/bulk-import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dbKey: "business_customers", records: rows }) });
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-56">
            ...
            <Input placeholder="Search by name, phone, or business…" value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center py-16 text-center gap-3">
              ...
              <p className="text-sm text-gray-500">No business customers yet</p>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Business Name</TableHead><TableHead>Business Type</TableHead>
                  <TableHead>Reg. No.</TableHead><TableHead>Company</TableHead>
              ...
              <TableBody>
                {visible.map((c, i) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      {c.businessType ? <span className="px-2 py-0.5 text-xs rounded-full bg-indigo-50 text-indigo-700">{c.businessType}</span> : "—"}
                    </TableCell>
                    <TableCell className="text-sm text-gray-500">{c.registrationNo || "—"}</TableCell>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="max-w-lg">
            <DialogHeader><DialogTitle>{editItem ? "Edit Customer" : "Add Business Customer"}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Contact Name *</label><Input value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} placeholder="e.g. John CEO" /></div>
                ...
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Business Name</label><Input value={form.businessName} onChange={e => setForm(f => ({...f, businessName: e.target.value}))} placeholder="e.g. Acme Ltd" /></div>
                <div>
                  <label className="text-xs font-medium text-gray-600 mb-1 block">Business Type</label>
                  <Select value={form.businessType || "__none__"} onValueChange={v => setForm(f => ({...f, businessType: v === "__none__" ? "" : v}))}>
                    <SelectTrigger><SelectValue placeholder="Select type" /></SelectTrigger>
                    <SelectContent><SelectItem value="__none__">— None —</SelectItem>{BUSINESS_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div><label className="text-xs font-medium text-gray-600 mb-1 block">Registration No.</label><Input value={form.registrationNo} onChange={e => setForm(f => ({...f, registrationNo: e.target.value}))} placeholder="e.g. REG-12345" /></div>
            ...
            <DialogFooter>
              ...
              <Button onClick={save} disabled={saving} className="bg-indigo-600 hover:bg-indigo-700 text-white">{saving ? "Saving…" : editItem ? "Save Changes" : "Add Customer"}</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={showBulk} onOpenChange={setShowBulk}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle className="flex items-center gap-2"><Upload className="w-5 h-5 text-indigo-600" /> Bulk Import Business Customers</DialogTitle></DialogHeader>
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-indigo-50 border border-indigo-100 rounded-lg">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-indigo-800">CSV Template</p>
                  <p className="text-xs text-indigo-600">Required: <strong>Name, Phone</strong>. Optional: Business Name, Business Type, Registration No, Contact Person, Email, Address, Notes</p>
                </div>
              ...
              <div>
                ...
                <textarea rows={5} value={bulkText} onChange={e => { setBulkText(e.target.value); parseBulk(e.target.value); }}
                  className="w-full border border-gray-200 rounded-lg p-3 text-sm font-mono resize-y focus:outline-none focus:ring-1 focus:ring-indigo-400" />
              </div>
                  ...
                  <div className="border border-gray-200 rounded-lg max-h-48 overflow-y-auto divide-y divide-gray-50">
                    {bulkPreview.map((p, i) => (
                      <div key={i} className="px-3 py-2">
                        ...
                        <p className="text-xs text-gray-500">{p.phone}{p.businessName ? ` · ${p.businessName}` : ""}{p.businessType ? ` · ${p.businessType}` : ""}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <DialogFooter>
              ...
              <Button onClick={submitBulk} disabled={bulkSaving || bulkPreview.length === 0 || !bulkCompany} className="bg-indigo-600 hover:bg-indigo-700 text-white">
