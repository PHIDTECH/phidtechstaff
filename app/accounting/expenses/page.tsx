...
import { Receipt, Plus, Search, CheckCircle, Clock, XCircle, DollarSign, Edit, Trash2, Eye, AlertCircle, Building2 } from "lucide-react";
...
const emptyForm = () => ({ userId: "", title: "", category: "Travel", amount: "", description: "" });
...
export default function AccountingExpensesPage() {
  ...
  // Show company column when viewing group (no specific company selected)
  ...
  const filtered = companyExpenses.filter(e => {
    const emp         = allStaff.find(u => u.id === e.userId);
  ...
  return (
    <MainLayout>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
            ...
            <p className="font-semibold text-gray-700">No expense claims found</p>
            <p className="text-sm text-gray-400">Switch company or adjust filters to see records.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {filtered.map(claim => {
                ...
                const approver = allStaff.find(u => u.id === claim.approvedBy);
                return (
                  <TableRow key={claim.id}>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <p className="font-medium text-gray-900 text-sm">{emp?.name ?? <span className="text-gray-400 italic">Unknown</span>}</p>
                    ...
                    {showCompanyCol && (
                      <TableCell>
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-xs text-gray-500">{getCompanyName(claim.companyId)}</span>
                        </div>
                      </TableCell>
                    )}
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    {showCompanyCol && <p className="text-xs text-blue-500">{getCompanyName(viewItem.companyId)}</p>}
                  </div>
                </div>
      ...
      <Dialog open={showDialog} onOpenChange={v => { if (!v) setShowDialog(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>{editItem ? "Edit Expense Claim" : "Add Expense Claim"}</DialogTitle></DialogHeader>
